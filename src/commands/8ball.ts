...
const cmdData = new SlashCommandBuilder()
	...
	.addBooleanOption(option =>
		option.setName('invisible')
			.setDescription('If true, only you will see Emperor\'s response. Default: false.'));

const cmdExecuter = async i => {
	const question: string = truncateString(i.options.getString('question')
		.replace(/\*/g, '')
		.replace(/_/g, '')
		.replace(/~~/g, '')
		.replace(/>\ /g, '>'), 130);
	...
	try {
		if (!question) {
			return replier.reply(title.error, 'You don\'t seem to have provided a valid question.', true);
		}

		return replier.reply(title.response, `You asked:\n**${question}**\n\nYour luck is:\n**${eightBall()}**`, invisible);
	}
	catch(e) {
