...
export class EmperorTitle {
  ...
  constructor(interaction: CommandInteraction) {
    const commandNameFirstCap = interaction.commandName.charAt(0).toUpperCase() + interaction.commandName.slice(1);

    const titleTemplate = !(interaction.options.getSubcommand(false) === null) ? `${commandNameFirstCap} ${interaction.options.getSubcommand()} ` : `${interaction.commandName} `;
