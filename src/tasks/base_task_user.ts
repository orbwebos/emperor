  ...
  async register(timeZone: string): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      if (!this.isRegistered()) {
        try {
          ensureDirectory(resolvePathFromSource(`../data/tasks/${this.id}`));
        } catch (e) {
          reject(e);
        }
      }
      ...
      const toWrite = {
        id: this.id,
        time_zone: timeZone,
        tasks_remind_by_default: false,
        group_by: 'none',
        remove_requires_options: true,
      };
        ...
        .then(async (response) => {
          if (response.includes('Wrote to')) {
            for (const i in contexts) {
              ...
              let doesIt: boolean;
              try {
                doesIt = fs.existsSync(folderPath);
              } catch (e) {
                reject(e);
              }
              ...
              if (!doesIt) {
                try {
                  ensureDirectory(
                    resolvePathFromSource(
                      `../data/tasks/${this.id}/${contexts[i]}`
                    )
                  );
                } catch (e) {
                  reject(e);
