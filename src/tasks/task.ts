...
export class Task {
  ...
  constructor(params: TaskCreationParameters) {
    const id = params.existing ? params.existing.id : randomBytes(3).toString('hex');
    ...
    if (isNullOrUndefined(params.deadlineReminderDaysBefore) || params.deadlineReminderDaysBefore < 0) {
      params.deadlineReminderDaysBefore = 1;
    }
    if (isNullOrUndefined(params.deadlineReminderInterval) || params.deadlineReminderInterval < 1) {
      params.deadlineReminderInterval = 15;
    }
    if (isNullOrUndefined(params.deadlineReminderOffset) || params.deadlineReminderOffset < 0) {
      params.deadlineReminderOffset = 3;
    }
    if (isNullOrUndefined(params.deadlineKeepRemindingFor) || params.deadlineKeepRemindingFor < 0) {
    ...
    if (!params.existing && !isNullOrUndefined(params.wakeIn) && isNullOrUndefined(params.asleep)) {
      params.asleep = true;
    }
    ...
    if (!isNullOrUndefined(params.description) && params.description.length > 250) {
      throw new Error('description can\'t exceed 250 characters');
    }

    if (!isNullOrUndefined(params.customId) && (isNullOrUndefined(params.existing) || (params.existing.editingIntention && params.existing.customId !== params.customId))) {
      let idMatchResponse: string = '';
      try {
        idMatchResponse = this.user.walkTreeUntilParamMatch(taskHelp.taskInPathMatchesAnyId, params.customId);
      }
      catch(e) {
    ...
    let oDate: {date: Date, dateString: string} = {date: null, dateString: null};
    let oDeadline: {date: Date, dateString: string} = {date: null, dateString: null};
    let oWakeIn: {date: Date, dateString: string} = {date: null, dateString: null};
    try {
      if (params.date) {
        oDate = taskHelp.validateAndGetTaskDate(params.date, this.user.config.time_zone);
      }
      if (params.deadline) {
        oDeadline = taskHelp.validateAndGetTaskDate(params.deadline, this.user.config.time_zone);
      }
      if (params.wakeIn) {
        oWakeIn = taskHelp.validateAndGetTaskDate(params.wakeIn, this.user.config.time_zone);
      }
    }
    catch(e) {
      throw new Error(e);
    ...
    const now = new Date();
    const trashAddedDate = params.existing ? new Date(params.existing.dateTrashed) : now;
    ...
    this.trash = {
      ...
      dateAdded: {
        ...
        string: params.markAsTrashed ? trashAddedDate.toISOString() : now.toISOString()
      },
      deletionDate: {
        date: params.markAsTrashed ? trashDeletionDate : now,
        string: params.markAsTrashed ? trashDeletionDate.toISOString() : now.toISOString()
      }
    };
  ...
  public updateImportance(): void {
    ...
    if (this.dates.deadline.reminder) { importance = importance + 250 };
    if (this.late) { importance = importance + 400 }

    if (this.priority === 1) { importance = importance + 1000 }
    else if (this.priority === 2) { importance = importance + 500 };
    ...
    switch (this.context.code) {
      ...
      case TaskContext.Other:
        ...
        break;
      default:
        throw new Error(`state error: invalid context code: ${this.context.code}`);
    }

    if (this.status.code === 'FEEDBACK') { importance = importance - 800 };
    if (this.status.code === 'DELEGATED') { importance = importance - 4000 };
    if (this.status.code === 'DONE') { importance = importance - 5000 };
    if (this.dormant) { importance = importance - 7000 }
    if (this.trash.isIn) { importance = importance - 20000 };
  ...
  shouldRemind(): boolean {
    return !this.trash.isIn && this.dates.deadline.reminder && this.status.code !== 'DONE';
  }
}
